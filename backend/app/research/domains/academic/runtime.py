...
from app.research.domains.academic.evidence import EvidenceCard, EvidenceExtractor, render_card
from app.research.domains.academic.prompts import SYSTEM_PROMPT, build_user_prompt
...
class AcademicRuntime:
    ...
    def _extract_search_queries(content: str, fallback_query: str, count: int) -> list[str]:
        tool_call = AcademicRuntime._extract_tool_call(content)
        queries: list[str] = []
    ...
    def _mark_search_candidates(
        ...
        for source in sources:
            ...
            enriched["search_id"] = AcademicRuntime._roman_numeral(len(searched_urls))
    ...
    def _extract_tool_call(content: str) -> Optional[dict[str, Any]]:
        tool_calls = AcademicRuntime._extract_tool_calls(content)
    ...
    def _extract_tool_calls(content: str) -> list[dict[str, Any]]:
        ...
        for match in TOOL_CALL_PATTERN.finditer(content):
            parsed = AcademicRuntime._parse_first_json_object(match.group(1).strip())
        ...
        parsed = AcademicRuntime._parse_first_json_object(raw)
    ...
    def _missing_report_format(
        ...
        *,
        ...
        body_chars = AcademicRuntime._count_report_body_chars(answer)
