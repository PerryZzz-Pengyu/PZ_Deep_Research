...
- 实现研究输入页。
- 实现研究模式选择。
  - 快速模式：1 个高命中英文搜索词，访问 3 个关键来源，500 字以内 essay 风格报告。
  - 深度模式：3 个高命中英文搜索词，访问 10 个关键来源，1500 字以内文献综述风格报告。
  - 专家模式：两次搜索，每次 5 个高命中英文搜索词，总共访问 20 个关键来源，3000 字以上论文风格报告。
- 实现实时研究进度时间线。
- 实现最终报告展示。
- 实现来源列表和引用展示。
- 后端提供任务创建、任务状态、事件流、报告获取等 API。

当前状态：待开始。下一步建议进入阶段 4，重点是把阶段 2/3 的事件和来源能力在网页端做成更完整的用户体验。
