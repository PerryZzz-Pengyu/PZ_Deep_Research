...
当前状态：Provider 抽象和 Runtime 工程闭环已完成，真实多模型产品联调尚未全部完成。
...
- Claude 和 Gemini 尚未完成真实 API Key 回归测试。
...
- 研究问题输入、Provider/模型选择和快速/深度/专家模式。
...
- Playwright 浏览器端到端测试：覆盖运行中取消、刷新后续接任务、完成后再次刷新恢复报告、历史报告详情和重新运行；测试固定使用 `8000/3000`。
...
- 重新运行：终态任务按原问题、模式、Provider 和模型创建独立新任务，并通过 `rerun_of_job_id` 记录来源任务；运行中任务拒绝重复重跑。
...
- 继续追问和更完整的错误恢复交互。
- OpenAI、Claude、Gemini 的可重复真实 API 回归测试矩阵。
- 关键桌面/移动视口的视觉回归验证。
- PostgreSQL 真实实例、备份恢复和多实例部署验收。
...
- OpenAI、Claude、Gemini 均通过至少 quick 模式真实 API 验收，OpenAI 额外通过 deep 模式回归。
...
当前状态：尚未开始正式实施。目前只有后端保存密钥、readiness 检查、用量事件等基础准备，不能视为产品化能力。
...
| 阶段 2：模型无关 Runtime | 基础完成 | Provider 抽象完成；Claude/Gemini 原生流式、真实联调和成本计算仍需补齐 |
| 阶段 3：工具层 | MVP 完成 | SerpAPI + Jina 主链路可用；备用读取链路和学术元数据增强未完成 |
| 阶段 4：网页端 MVP | 部分完成 | 取消、刷新恢复、持久化、匿名历史、报告详情、重跑、Markdown/PDF 导出和核心 E2E 已完成；真实多模型验收尚未完成 |
| 阶段 5：产品化 | 尚未开始 | 登录、配额、队列、安全、监控和部署仍待实施 |

## 下一实施里程碑

在继续增加高级研究能力前，优先完成阶段 4 收尾：

1. 为完整错误恢复定义测试与验收标准并完成交互。
2. 建立 OpenAI、Claude、Gemini 真实 Provider 回归测试矩阵。
3. 在真实 PostgreSQL 实例上验证迁移、连接和备份恢复。
4. 增加关键视口视觉回归，并在其余退出条件完成后关闭阶段 4。
