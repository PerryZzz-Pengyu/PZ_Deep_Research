...
- 同一后端进程内的刷新恢复：浏览器保存当前任务 ID，刷新后恢复问题、模式、Provider、时间线、来源、报告草稿和最终报告。
- SSE 续接：使用持久事件游标、任务快照和累计报告草稿恢复连接，避免重复历史事件或重复拼接报告 delta。
- Playwright 浏览器端到端测试：覆盖运行中取消、刷新后续接任务、完成后再次刷新恢复报告；测试固定使用 `8000/3000`。

阶段 4 尚未完成：

- 任务历史与报告详情：当前左侧“历史”只是界面入口，没有任务列表、详情恢复和后端列表 API。
- 持久化：任务、事件和报告草稿仍保存在进程内存中，后端重启后会丢失；当前刷新恢复只覆盖后端进程仍存活的场景。
- 保存、重新运行、继续追问和错误恢复交互。
...
| 阶段 4：网页端 MVP | 部分完成 | 取消、同进程刷新恢复和核心 E2E 已完成；历史、持久化、重跑、导出和真实多模型验收尚未完成 |
...
1. 为历史记录、持久化、重新运行和 Markdown 导出定义测试与验收标准。
2. 引入持久化存储并增加任务列表、任务详情和重新运行 API。
3. 完成前端历史、详情、重跑、导出和完整错误恢复交互。
4. 建立 OpenAI、Claude、Gemini 真实 Provider 回归测试矩阵。
