...
涉及定价、成本、利润、供应商预算和投放策略的商业资料属于私有信息，只保存在被 `.gitignore` 排除的本地文档中，不进入公开仓库。公开项目文档只记录产品能力和工程阶段，不记录可反推出经营策略的参数。
...
- 实现研究输入页。
- 实现研究模式选择。
  - 快速模式：1 个高命中英文搜索词，最终选择 3 个关键来源，正文 350-900 字。
  - 深度模式：3 个高命中英文搜索词，最终选择 10 个关键来源，正文 1100-2600 字。
  - 专家模式：强制两次搜索，每次 5 个高命中英文搜索词，从两轮访问并集中最终选择 20 个关键来源，正文 2700-5200 字。
- 实现实时研究进度时间线。
- 实现最终报告展示。
- 实现来源列表和引用展示。
...
- Playwright 浏览器端到端测试：11 个用例覆盖运行中取消、刷新后续接任务、完成后再次刷新恢复报告、侧栏历史、重新运行、产品化错误重试、Markdown/PDF 导出、Clerk 初始化失败访客降级、HeroUI Tabs、恢复超时和移动端来源 Modal；在 `/workbench` 路由上运行、重点测英文界面。默认使用 `8000/3000`，也可通过 Playwright 环境变量使用隔离端口。
...
- 正式接入 HeroUI v3.1.0 与 Tailwind CSS v4，基础交互使用官方 React 组件；PZ 保留深色玻璃品牌视觉、研究业务组件和中 / 英多语言。新增营销落地页（`/`），研究工作台位于 `/workbench`。
...
当前状态：已开始实施。Clerk 登录、后端 JWT 验签、匿名历史认领、账号级隔离和跨设备历史基础链路已完成；**用量计量地基已落地**（每任务用量聚合 + 按账号 `GET /api/usage`，公开仓只存原始计数，成本/定价属私有 Cloud），可作为后续额度展示与计费的根；额度、支付、队列、安全、监控和正式部署仍未完成。具体商业参数只在本地私有文档维护。
...
- 增加可审计的额度记录、任务结算和支付一致性机制；具体商业规则不在公开文档展开。
...
在继续增加高级研究能力前，优先完成阶段 4 收尾和阶段 5 产品化底座：
