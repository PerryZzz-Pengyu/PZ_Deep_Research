...
- Playwright 浏览器端到端测试：覆盖运行中取消、刷新后续接任务、完成后再次刷新恢复报告；测试固定使用 `8000/3000`。
- SQLite/PostgreSQL 持久化：任务、持久事件、报告草稿和最终报告写入数据库，后端重启后仍可查询。
- Alembic 初始迁移和 PostgreSQL 兼容迁移 SQL。
- 基础历史记录：当前按浏览器匿名访客 ID 隔离，点击历史任务可以恢复详情。
- 数据归属预留 `user_id`，未来登录时可将匿名访客历史归并到账号。

阶段 4 尚未完成：

- 保存、重新运行、继续追问和错误恢复交互。
- Markdown 导出；PDF 导出可以放到阶段 5。
...
- 增加用户级历史记录和跨设备数据同步。
- 增加额度和使用量统计。
- 在阶段 4 基础 Markdown 导出之上增加 PDF 和更完整的导出能力。
- 增加内容安全和滥用控制。
- 增加部署配置。
...
| 阶段 4：网页端 MVP | 部分完成 | 取消、刷新恢复、SQLite/PostgreSQL 兼容持久化、匿名历史和核心 E2E 已完成；重跑、导出和真实多模型验收尚未完成 |
...
1. 为重新运行、Markdown 导出和完整错误恢复定义测试与验收标准。
2. 完成前端重跑、导出和完整错误恢复交互。
3. 建立 OpenAI、Claude、Gemini 真实 Provider 回归测试矩阵。
4. 在真实 PostgreSQL 实例上验证迁移、连接和备份恢复。
5. 增加关键视口视觉回归，并在其余退出条件完成后关闭阶段 4。
