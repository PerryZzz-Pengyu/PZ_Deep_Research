...
当前状态：Provider 抽象和 Runtime 工程闭环已完成，三家 Provider 已完成不同程度的真实调用验证；分阶段质量测试和生产模型路由尚未完成。
...
- 研究问题输入、开发测试用 Provider/模型选择和快速/深度/专家模式。
...
- 重新运行：终态任务按原问题、模式和当前内部 Provider/模型配置创建独立新任务，并通过 `rerun_of_job_id` 记录来源任务；运行中任务拒绝重复重跑。生产后台路由完成后，应改为保存并复用路由版本。
...
暂不建立以“每个 Provider × 每个研究模式”为中心的全面回归矩阵。下一阶段优先做任务级质量测试，分别选择最适合以下职责的模型：
...
质量测试可以使用 OpenAI、Claude、Gemini 的候选模型，但测试结论服务于后台模型编排，不向 C 端用户提供多模型切换。生产任务由 Runtime 按职责自动选择模型；模型名称和路由版本仍写入任务日志，便于质量追踪、成本分析和故障回溯。
...
| 阶段 4：网页端 MVP | 部分完成 | 取消、刷新恢复、持久化、匿名历史、报告详情、重跑、产品化错误恢复、Markdown/PDF 导出和核心 E2E 已完成；分阶段模型质量测试与生产路由尚未完成 |
...
1. 创建 Neon 项目，在真实 PostgreSQL 实例上验证迁移、连接池、备份和恢复。
2. 建立意图识别、工具规划、证据卡片、报告撰写四类质量测试集，确定每个环节的生产模型。
3. 移除生产 C 端的 Provider/模型选择入口，改为后端路由；内部测试环境继续保留切换能力。
