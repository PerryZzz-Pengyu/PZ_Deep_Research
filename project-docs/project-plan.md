...
- Playwright 浏览器端到端测试：6 个用例覆盖运行中取消、刷新后续接任务、完成后再次刷新恢复报告、历史报告详情、重新运行、Markdown 导出和 PDF 导出；测试固定使用 `8000/3000`。
...
| 阶段 4：网页端 MVP | 部分完成 | 取消、刷新恢复、持久化、匿名历史、报告详情、重跑、Markdown/PDF 导出和核心 E2E 已完成；分阶段模型质量测试与生产路由尚未完成 |
...
4. 移除生产 C 端的 Provider/模型选择入口，改为后端路由；内部测试环境继续保留切换能力。
